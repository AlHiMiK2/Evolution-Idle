...
namespace _Project.Scripts.Animations
{
    ...
    public class BunnyAnimation : MonoBehaviour
    {
        ...
        [SerializeField] private BunnyMoveState _moveState;
        [SerializeField] private BunnyAttackState _attackState;
        [SerializeField] private BunnyAttackedState _attackedState;
        ...
        private void Update()
        {
            ...
            _animator.SetBool(IsMove, _moveState.enabled);
            _animator.SetBool(IsAttack, _attackState.enabled);
            _animator.SetBool(IsHit, _attackedState.enabled);
            _renderer.flipX = _data.Direction.x < 0;

            if (_attackedState.enabled && !_isHitEffect)
            {
                StartHitEffect();
            }
            else if (!_attackedState.enabled && _isHitEffect)
            {
                StopHitEffect();
            }
        }
