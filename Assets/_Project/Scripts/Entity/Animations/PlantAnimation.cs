...
namespace _Project.Scripts.Animations
{
    ...
    public class PlantAnimation : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (_plant.IsAttacked && !_isHitEffect)
            {
                StartHitEffect();
            }
            else if (!_plant.IsAttacked && _isHitEffect)
            {
                StopHitEffect();
            }
        }
        ...
        private void OnDied()
        {
            _shadow.DOFade(0, _plant.DieDelay).SetEase(_ease);
            _renderer.transform.DOMoveY(_offsetY, _plant.DieDelay).SetEase(_ease).SetRelative(true);
            _renderer.transform.DOScale(_dieScale, _plant.DieDelay).SetEase(_ease);
            _material.DOFloat(1f, HitEffectBlend, _plant.DieDelay).SetEase(_ease);
            _renderer.DOFade(0, _plant.DieDelay * 0.5f).SetEase(_ease).SetDelay(_plant.DieDelay * 0.5f);
        }
    }
}